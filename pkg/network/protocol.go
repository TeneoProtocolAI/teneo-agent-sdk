...
func NewProtocolHandler(client *NetworkClient, authManager *auth.Manager, agentName string, capabilities []string, walletAddr string) *ProtocolHandler {
...
func (p *ProtocolHandler) HandleAuthResponse(msg *types.Message) error {
	if strings.Contains(msg.Content, "successful") {
		...
		// Only send capabilities, like x-agent does (no separate registration)
		return p.SendCapabilities()
	} else {
...
func (p *ProtocolHandler) HandleTask(msg *types.Message) error {
	...
	if err := json.Unmarshal(msg.Data, &taskData); err != nil {
		...
		return p.processTask(msg.From, msg.Content, "")
	}
	...
	return p.processTask(msg.From, taskContent, taskID)
}

// processTask processes a task and sends a response
func (p *ProtocolHandler) processTask(from, content, taskID string) error {
...
// SendTaskResponse sends a task response back to the coordinator
func (p *ProtocolHandler) SendTaskResponse(taskID, content string, success bool, errorMsg string) error {
	responseData := map[string]interface{}{
		"task_id": taskID,
		"success": success,
	}
